...
[@bs.val] [@bs.module "papaparse"] external parseString: string => Results.t = "parse";
...
let lineReducer = (acc: list(order), items: array(string)): list(order) =>
  if (Belt.Array.length(items) != 7) {
    ...
    let orderRecord =
      map2(initial, optInt(items[0]), (result, n) => {...result, quantity: n})
      ->map2(Shirt.Size.fromString(items[1]), (result, sz) => {...result, size: sz})
      ->map2(Shirt.Color.fromString(items[2]), (result, c) => {...result, color: c})
      ->map2(Shirt.Pattern.fromString(items[3]), (result, pat) => {...result, pattern: pat})
      ->map2(Shirt.Collar.fromString(items[4]), (result, coll) => {...result, collar: coll})
      ->map2(Shirt.Sleeve.fromString(items[5]), (result, sleeve) => {...result, sleeve})
      ->map2(Shirt.Cuff.fromString(items[6]), (result, cuff) => {...result, cuff});
...
let makeObject =
    (title: string, distribution: Belt.Map.t('k, 'v, 'id), toString: 'a => string): Js.Json.t => {
...
let processFile = (inFileName: string, column: string): Js.Json.t => {
  ...
  let lines = Belt.Array.slice(parseData, ~offset=1, ~len=Belt.Array.length(parseData) - 1);
